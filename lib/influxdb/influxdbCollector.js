  ...
  var aggregateNames = ['min','p10', 'p25', 'median', 'p70', 'p75', 'p80', 'p90', 'p99', 'max', 'sum', 'mean', 'stddev'];
	...
	{ 
		...
		if( metric.externalID )
			dataTags += ",externalID=" + metric.externalID; // needed for WebPageTest. See aggregators/webpagetest/genericTimeMetric.js
		...
		aggregateNames.forEach( function(aggregateName) 
		{
			...
			if( aggregateValue < 0 || isNaN(aggregateValue) || aggregateValue === undefined || aggregateValue == null)
			{
				self.log.info("Metric " + metricName + ":"+aggregateName+" was < 0! setting to -1 manually " + aggregateValue + " || " + JSON.stringify(aggregateValue));
				aggregateValue = -1;
			}
		...
		if( dataFields != "" )
		{
			dataFields += "count=" + self.config.noReal; // .noReal = (self.config.no - self.config.skipXruns);
