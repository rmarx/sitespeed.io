...
WPTAggregator.prototype._collectData = function(run, view, browserAndLocation) {
  ...
  if(browserAndLocation.response.data.connectivity )
   connectivity = (browserAndLocation.response.data.connectivity).toLowerCase();
...
WPTAggregator.prototype.generateResults = function() {
  ...
  return []; // explicitly for PROFLOW project, since we gather all these metrics in our own genericTimeMetric.js aggregator
