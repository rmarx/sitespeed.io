...
exports.setupDefaultValues = function(config) {
  ...
  // put skipXruns in [0, config.no - 1]
  config.skipXruns = Math.min( config.skipXruns, config.no - 1 ); // keep at least 1 real run
  config.skipXruns = Math.max( 0, config.skipXruns );
  config.noReal = config.no;
  if( config.skipXruns > 0 )
  {
  	config.no += config.skipXruns;
  }
