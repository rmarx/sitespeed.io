...
var cli = nomnom.help(
	...
	influxdbHost: {
		metavar: '<HOST>',
		help: 'The Influxdb host (no default).'
	},
	influxdbPort: {
		metavar: '<INTEGER>',
		default: defaultConfig.influxdbPort,
		help: 'The Influxdb port (default: '+ defaultConfig.influxdbPort +').'
	},
	influxdbDatabase: {
		metavar: '<DATABASE>',
	...
	influxdbTags: {
		metavar: '<TAGS>',
		default: defaultConfig.influxdbTags,
		help: 'First part of the influxdb plaintext protocol, i.e. measurement_name,tagname=tagvalue'
	},
	skipXruns: {
		metavar: '<INTEGER>',
		default: 0,
		help: 'Skip the first x runs (usually the first run has wrong timing data due to cold OS-level caches etc.). These skipped runs are separate from the total run count (-n or --no)! So -n 5 and --skipXruns 3 will perform 8 runs total'
	},
	gpsiKey: {
	...
	wptConfig: {
		...
		transform: function(path) 
		{
			var stringPath = "" + JSON.stringify(path); // for some reason, path is an Object here... 
			if( stringPath[0] == '{' || (stringPath[0] == '\"' && stringPath[1] == '{') )
			{
				return JSON.parse(stringPath);
			}
			else
				return fileHelper.getFileAsJSON(path);
		}
	...
	wptScript: {
		metavar: '<FILE>|<BASE64STRING>',
		help: 'WebPageTest scripting. Every occurance of {{{URL}}} will be replaced with the real URL. Path to file or base64 encoded string. File needs to have an extension (. character in path)',
		callback: function(path) 
		{
			if( path.indexOf(".") > 0 ) // path should include file extension (i.e., .txt or .json)
			{
		...
		transform: function(path) 
		{
			...
			if( stringPath.indexOf(".") > 0 ) // path should include file extension (i.e., .txt or .json)
			{
				var fileContents = fileHelper.getFileAsString(path);
				//console.log( fileContents );
				return fileContents;
			}
			else
			{
				var decoded = new Buffer(stringPath, 'base64'); //atob(stringPath);
