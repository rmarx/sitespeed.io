...
AnalyzeOneSite.prototype._verifyURL = function(callback) {
  ...
  request(options, function(error, response) {
	...
	var errorMessage = "" + (response ? response.statusCode : error);
      self.log.error('Couldn\'t fetch ' + self.config.url + ' ' + errorMessage);
	if( errorMessage.indexOf("self signed certificate") >= 0 )
	{
		self.log.error("but still continuing because we ignore self signed cert errors");
      		callback(null, [self.config.url], []);
	}
	else if( errorMessage.indexOf("Hostname/IP doesn't match certificate's altnames") >= 0 )
	{		
		self.log.error("but still continuing because we ignore self signed cert errors");
      		callback(null, [self.config.url], []);
	}
...
AnalyzeOneSite.prototype._analyze = function(urls, downloadErrors, callback) {
  var analysisErrors = {};
  var self = this;
  this.log.log('info', 'Will analyze ' + urls.length + (urls.length === 1 ? ' page' : ' pages') + " " + this.config.no + " times" + (this.config.skipXruns > 0 ? ", skipping the first " + this.config.skipXruns + " runs." : ".") );
