	...
	analysisResults.forEach(function(result) 
	{
		...
		for (url in data) 
		{
			...
			if (data.hasOwnProperty(url)) 
			{
				...
				if( config.skipXruns && config.skipXruns > 0 && pageData[result.type].runs && pageData[result.type].runs.length > 1 )
				{
					var topdata = pageData[result.type];
				
					// for some reason, pageData contains the same data twice...
					// once in pageData[result.type].metricName for all metrics (Stats object)
					// then again in pageData[result.type].runs for each run individually (individual measurements, later added to Stats by aggregator)
					...
					for( var property in topdata )
					{
						...
						{
							console.log( "analyzer.js > shifting measurement " + property );
